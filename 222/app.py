...
def get_working_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Возвращает данные с локальными правками таблицы и добавленными пунктами."""
    edited_records = st.session_state.get("edited_full_records")
    if edited_records is not None:
        df = normalize_dataframe(pd.DataFrame(edited_records))

    added_records = st.session_state.get("added_records", [])
    if added_records:
        added_df = normalize_dataframe(pd.DataFrame(added_records))
        df = pd.concat([df, added_df], ignore_index=True, sort=False)

    return add_unit_display(df)


def restore_canonical_dataframe(edited_df: pd.DataFrame) -> pd.DataFrame:
    """Преобразует таблицу с русскими заголовками обратно в CSV-структуру приложения."""
    reverse_labels = {label: column for column, label in TABLE_LABELS.items()}
    restored = edited_df.rename(columns={column: reverse_labels.get(str(column), column) for column in edited_df.columns}).copy()

    for column in CANONICAL_COLUMNS:
        if column not in restored.columns:
            restored[column] = ""

    restored = restored.loc[:, CANONICAL_COLUMNS]
    has_any_value = restored.fillna("").astype(str).apply(
        lambda row: any(value.strip() for value in row),
        axis=1,
    )
    restored = restored.loc[has_any_value].copy()
...
def sidebar_filters(df: pd.DataFrame) -> dict:
    ...
    # После добавления нового пункта важно сбросить фильтры до стартового состояния,
    # иначе новая запись может быть скрыта старым поиском, районом или выбранным вопросом.
    if st.session_state.pop("reset_filters_after_add", False):
        st.session_state["filter_regions"] = regions
        st.session_state["filter_districts"] = []
        st.session_state["filter_question"] = "Все вопросы"
        st.session_state["filter_unit_query"] = ""
        st.session_state["filter_text_query"] = ""
        st.session_state["filter_color_mode"] = "Диалектные единицы"

    previous_regions = list(st.session_state.get("_filter_region_options", []))
    selected_before = list(st.session_state.get("filter_regions", []))
    if "filter_regions" not in st.session_state:
        st.session_state["filter_regions"] = regions
    elif set(selected_before) == set(previous_regions):
        # Если раньше были выбраны все регионы, новый регион тоже должен стать выбранным.
        st.session_state["filter_regions"] = regions
    else:
        st.session_state["filter_regions"] = [region for region in selected_before if region in regions]
    st.session_state["_filter_region_options"] = regions

    selected_regions = st.sidebar.multiselect("Регионы", regions, key="filter_regions")
    ...
    if "filter_districts" not in st.session_state:
        st.session_state["filter_districts"] = []
    else:
        st.session_state["filter_districts"] = [district for district in st.session_state["filter_districts"] if district in districts]
    selected_districts = st.sidebar.multiselect("Районы / округа", districts, key="filter_districts")
    ...
    if "filter_question" not in st.session_state or st.session_state["filter_question"] not in question_labels:
        st.session_state["filter_question"] = "Все вопросы"
    selected_label = st.sidebar.selectbox("Карта / вопрос", question_labels, key="filter_question")

    unit_query = st.sidebar.text_input(
        "Поиск лингвистической единицы",
        placeholder="например: [ɣ], ляда, -ут",
        key="filter_unit_query",
    )
    text_query = st.sidebar.text_input(
        "Общий поиск",
        placeholder="пункт, район, комментарий",
        key="filter_text_query",
    )
    color_mode = st.sidebar.selectbox(
        "Раскраска точек",
        ["Диалектные единицы", "Ландшафт", "Тип вопроса", "Атлас"],
        key="filter_color_mode",
    )
    show_areals = st.sidebar.checkbox("Показывать ареалы", value=True, key="filter_show_areals")
    show_isoglosses = st.sidebar.checkbox("Показывать изоглоссы", value=True, key="filter_show_isoglosses")
    show_labels = st.sidebar.checkbox("Подписи пунктов", value=False, key="filter_show_labels")
...
def render_add_settlement_block(df: pd.DataFrame) -> None:
    ...
    add_message = st.session_state.pop("add_settlement_message", "")
    if add_message:
        st.success(add_message)

    added_count = len(st.session_state.get("added_records", []))
    if added_count:
        info_col, clear_col = st.columns([3, 1])
        with info_col:
            st.info(f"В текущей сессии добавлено записей: {added_count}. Они уже включены в карту, таблицу и CSV для скачивания.")
        with clear_col:
            if st.button("Очистить добавленные", use_container_width=True):
                st.session_state["added_records"] = []
                st.session_state["add_settlement_message"] = "Добавленные в текущей сессии записи очищены."
                st.session_state["reset_filters_after_add"] = True
                st.rerun()

    st.caption("Координаты можно найти автоматически по названию, району и региону. Запись добавляется в текущую сессию и сразу появляется на карте и в таблице.")
    ...
    if st.button("Добавить запись"):
        try:
            ...
            added_records = list(st.session_state.get("added_records", []))
            added_records.append(record)
            st.session_state["added_records"] = added_records
            st.session_state["reset_filters_after_add"] = True
            st.session_state["add_settlement_message"] = (
                f"Пункт «{record['settlement']}» добавлен. Карта, таблица и текущая полная CSV обновлены."
            )
            st.rerun()
        except ValueError as exc:
            st.error(str(exc))
...
def render_table_section(df: pd.DataFrame, filtered: pd.DataFrame, editor_url: str) -> None:
    ...
    table_message = st.session_state.pop("table_message", "")
    if table_message:
        st.success(table_message)
    ...
    st.caption("Видимую выборку можно редактировать и скачать. Чтобы правки реально попали в карту и текущую полную CSV, используйте полный редактор ниже и кнопку «Применить правки полной таблицы».")
    ...
    with st.expander("Шаблон и полная копия", expanded=False):
        ...
        st.caption("Здесь можно добавлять новые строки, менять координаты и править поля. После редактирования нажмите кнопку применения, иначе карта не изменится.")
        full_editor = display_dataframe(df, CANONICAL_COLUMNS)
        edited_full_editor = st.data_editor(
            full_editor,
            num_rows="dynamic",
            use_container_width=True,
            hide_index=True,
        ...
        with apply_col:
            if st.button("Применить правки полной таблицы", type="primary", use_container_width=True):
                restored_df = restore_canonical_dataframe(edited_full_editor)
                st.session_state["edited_full_records"] = restored_df.to_dict("records")
                st.session_state["added_records"] = []
                st.session_state["reset_filters_after_add"] = True
                st.session_state["table_message"] = "Правки полной таблицы применены. Карта, фильтры и CSV обновлены."
                st.rerun()
        with reset_col:
            if st.session_state.get("edited_full_records") is not None:
                if st.button("Сбросить локальные правки", use_container_width=True):
                    st.session_state.pop("edited_full_records", None)
                    st.session_state["added_records"] = []
                    st.session_state["reset_filters_after_add"] = True
                    st.session_state["table_message"] = "Локальные правки сброшены. Загружены исходные данные."
                    st.rerun()
