...
table.clean-table tr:nth-child(even) td { background: #fbfdff; }
.badge {
...
def render_light_dataframe(df: pd.DataFrame, height: int = 430, max_rows: int = 500) -> None:
    ...
    st.markdown(
        f"<div class='clean-table-wrap' style='max-height:{height}px'><table class='clean-table'><thead><tr>{header}</tr></thead><tbody>{''.join(rows)}</tbody></table></div>{note}",
        unsafe_allow_html=True,
    )
...
def render_legend(points_df: pd.DataFrame, areals: list[dict]) -> None:
    ...
    legend_html = [f"<p><b>{html.escape(summary)}</b></p>"]
    for _, row in labels.head(12).iterrows():
        label = str(row["color_label"])
        color = label_color_hex(label)
        legend_html.append(
            f"<span class='badge'><span style='display:inline-block;width:10px;height:10px;border-radius:50%;background:{color};margin-right:6px'></span>"
            f"{html.escape(label)} — {int(row['points'])}</span>"
        )
    if len(labels) > 12:
        legend_html.append(f"<p>Показано 12 из {len(labels)} значений.</p>")
    if areals:
        legend_html.append("<p>Ареалы построены по текущей выборке, включая общий поиск.</p>")
    with st.popover("Легенда") if hasattr(st, "popover") else st.expander("Легенда", expanded=True):
...
def render_points_page(df: pd.DataFrame) -> None:
    ...
    with c1:
        region = st.selectbox("Регион", ["Все регионы"] + sorted(map_df["region"].unique()))
...
Точки показывают только не городские населённые пункты. При наведении видны вопрос, единицы,
ландшафт и комментарий.
...
Во вкладке **Таблица** откройте блок **Добавить населённый пункт**. Если координаты не введены,
...
"""
    ...
    render_light_dataframe(pd.DataFrame({"question_type": ALLOWED_QUESTION_TYPES}), height=260)
...
def main() -> None:
    ...
    with tabs[4]:
        render_table_page(df, editor_url)
    with tabs[5]:
        render_gis_page()
    with tabs[6]:
        render_help_page()
